...
object IntegrationActors extends App {
  implicit val system = ActorSystem("IntegrationActors")
  ...
  val actorPoweredFlow =  Flow[Int].ask[Int](parallelism = 4)(simpleActor)
  ...
  val materializedActorRef =  actorPoweredSource.to(Sink.foreach[Int](number => println(s"Actor powered flow: $number"))).run()
...
//  materializedActorRef ! akka.actor.Status.Success("complete")
  ...
  class DestinationActor extends Actor with ActorLogging{
    override def receive: Receive = {
      ...
      case message =>
        log.info(s"Message Recieved $message")
        sender() ! StreamAck
    }
  }
